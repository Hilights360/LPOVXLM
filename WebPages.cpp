...
<style>
  ...
  html += "<h3>Arm Display Window</h3>"
          "<label>Lit Duration: <span id='dispv'>" + String(displayTimePercent) + "%</span></label>"
          "<input id='disp' type='range' min='0' max='100' value='" + String(displayTimePercent) + "'>"
          "<div class='row'><button id='applydisp'>Apply</button></div>"
          "<p class='muted'>Percentage of each spoke that remains illuminated before the arms blank.</p>"
          "<div class='sep'></div>";
...
const disp=document.getElementById('disp'), dispv=document.getElementById('dispv');
if(disp){disp.oninput=()=>dispv.textContent=disp.value+'%';}
...
const applydisp=document.getElementById('applydisp');
if(applydisp){applydisp.onclick=()=>{if(disp) post('/armwindow?percent='+disp.value);};}
