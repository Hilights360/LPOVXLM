...
String rootPage(const String &statusClass,
  ...
  String watchdogHelp = String("Reboots automatically if the controller locks up (e.g. SD card stall).");
  ...
  String html =
      ...
      "if(watchdog){watchdog.onchange=()=>{const en=watchdog.checked?'1':'0';fetch('/watchdog?enable='+en,{method:'POST'}).then(r=>{if(!r.ok){watchdog.checked=!watchdog.checked;}}).catch(()=>{watchdog.checked=!watchdog.checked;});};}"
