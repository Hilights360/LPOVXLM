...
String buildUploadTemplate(const String& message, const String& backUrl, unsigned refreshSeconds) {
  String html;
  html.reserve(message.length() + 128);
...
String buildControlPage(const ControlPageData& data) {
  const String current = data.currentPath.length() ? data.currentPath : String("(none)");
  const String statusClass = data.statusClass ? String(data.statusClass) : String();
  const String statusText = data.statusText ? String(data.statusText) : String();

  String html;
  html.reserve(3500 + data.optionsHtml.length());
  ...
  html += "POV Spinner";
  html += "<span id='status' class='" + statusClass + "'>" + statusText + "</span>";
  html += "<span id='which' class='pill'>" + htmlEscape(current) + "</span>";
  html += "</h1>";
  ...
  html += "<select id='sel'>" + data.optionsHtml + "</select>";
  html += "<div class='row'>";
  ...
  html += "<div><label>Start Channel (Arm 1)</label><input id='startch' type='number' min='1' value='" + String(data.startChannel) + "'></div>";
  html += "<div><label>Total Spokes</label><input id='spokes' type='number' min='1' value='" + String(data.totalSpokes) + "'></div>";
  html += "<div><label>Arm Count</label><input id='arms' type='number' min='1' max='" + String((int)data.maxArms) + "' value='" + String((int)data.armCount) + "'></div>";
  html += "<div><label>Pixels per Arm</label><input id='pixels' type='number' min='1' max='" + String((int)data.maxPixelsPerArm) + "' value='" + String(data.pixelsPerArm) + "'></div>";
  html += "<div><label>Stride</label><select id='stride'><option value='spoke' ";
  html += (data.strideMode == STRIDE_SPOKE ? "selected" : "");
  html += ">SPOKE</option><option value='led' ";
  html += (data.strideMode == STRIDE_LED ? "selected" : "");
  html += ">LED</option></select></div>";
  ...
  html += "<label>FPS: <span id='fpsv'>" + String(data.fps) + "</span></label>";
  html += "<input id='fps' type='range' min='1' max='120' value='" + String(data.fps) + "'>";
  ...
  html += "<label>Value: <span id='v'>" + String(data.brightnessPercent) + "%</span></label>";
  html += "<input id='rng' type='range' min='0' max='100' value='" + String(data.brightnessPercent) + "'>";
  ...
  html += "fetch('/mapcfg?start='+sc+'&spokes='+sp+'&arms='+ar+'&pixels='+px+'&stride='+st,{method:'POST'})";
...
String buildFilesPage(const FilesPageData& data) {
  String html;
  html.reserve(2500 + data.rowsHtml.length());
  html += "<!doctype html><html><head><meta charset='utf-8'>";
  html += "<meta name='viewport' content='width=device-width,initial-scale=1'>";
  html += "<title>Files - " + htmlEscape(data.currentPath) + "</title>";
  html += "<style>";
  ...
  html += "</div>";
  html += "<p>Path: <b>" + htmlEscape(data.currentPath) + "</b> &middot; ";
  html += "<a href='/files?path=" + data.parentPathEncoded + "'>Up</a></p>";
  html += "<form method='POST' action='/upload?dir=" + data.currentPathEncoded + "&back=" + data.uploadBackEncoded + "' enctype='multipart/form-data'>";
  html += "<input type='file' name='f' accept='.fseq' required> ";
  ...
  html += "<button onclick=\"const n=prompt('New folder name'); if(n) location='/mkdir?path=" + data.currentPathEncoded + "&name='+encodeURIComponent(n);\">New Folder</button>";
...
String buildDirectoryRow(const String& fullName, const String& currentPathEncoded) {
  ...
  const String esc = htmlEscape(fullName);
  const String enc = urlEncode(fullName);
  const String baseEsc = htmlEscape(base);
  ...
  html += "<td>—</td>";
  html += "<td>";
  html += "<a href='#' onclick=\"if(confirm('Delete folder " + baseEsc + "? (must be empty)')) location='/rm?path=" + enc + "&back=/files?path=" + currentPathEncoded + "'; return false;\">🗑️ Delete</a> &nbsp; ";
  html += "<a href='#' onclick=\"const n=prompt('Rename folder to:', '" + baseEsc + "'); if(n) location='/ren?path=" + enc + "&to='+encodeURIComponent(n)+'&back=/files?path=" + currentPathEncoded + "'; return false;\">✏️ Rename</a>";
  html += "</td></tr>";
  return html;
}
...
String buildFileRow(const String& fullName, uint64_t size, const String& currentPathEncoded) {
  ...
  html.reserve(400);
  html += "<tr><td>📄 " + esc + "</td>";
  html += "<td>" + String((unsigned long)size) + "</td>";
  html += "<td>";
  html += "<a href='/dl?path=" + enc + "'>⬇️ Download</a> &nbsp; ";
  html += "<a href='/play?path=" + enc + "&back=/files?path=" + currentPathEncoded + "'>▶️ Play</a> &nbsp; ";
  html += "<a href='#' onclick=\"if(confirm('Delete " + esc + "?')) location='/rm?path=" + enc + "&back=/files?path=" + currentPathEncoded + "'; return false;\">🗑️ Delete</a> &nbsp; ";
  html += "<a href='#' onclick=\"const n=prompt('Rename file to:', '" + baseEsc + "'); if(n) location='/ren?path=" + enc + "&to='+encodeURIComponent(n)+'&back=/files?path=" + currentPathEncoded + "'; return false;\">✏️ Rename</a>";
  html += "</td></tr>";
  return html;
}

String buildUploadRejectedPage(const String& backUrl) {
  return buildUploadTemplate("<p>Upload rejected. Only <b>.fseq</b> files are allowed.</p><p>Returning…</p>", backUrl, 2);
}

String buildUploadFailedPage(const String& backUrl) {
  return buildUploadTemplate("<p>Upload failed.</p><p>Returning…</p>", backUrl, 3);
}

String buildUploadSuccessPage(const String& filename, size_t bytes, const String& backUrl) {
  String message = "<p>Uploaded <b>" + htmlEscape(filename) + "</b> (" + String(bytes) + " bytes).</p><p>Refreshing…</p>";
  return buildUploadTemplate(message, backUrl, 1);
}
