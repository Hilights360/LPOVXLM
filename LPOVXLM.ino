...
static void handleFiles() {
  ...
  std::vector<FilesPageEntry> entries;
  File ent;
  while ((ent = dir.openNextFile())) {
    FilesPageEntry item;
    item.name = String(ent.name());
    item.isDirectory = ent.isDirectory();
    item.size = item.isDirectory ? 0 : ent.size();
    entries.push_back(item);
    ent.close();
  }
  dir.close();

  FilesPageContext ctx;
  ctx.path = path;
  ctx.parentPath = dirnameOf(path);
  ctx.entries = &entries;

  const String html = buildFilesPage(ctx);
...
static void handleRoot() {
  ...
  ControlPageContext ctx;
  ctx.optionsHtml = options;
  ctx.currentPath = g_currentPath.length() ? g_currentPath : "(none)";
  ctx.statusClass = String(statusClass());
  ctx.statusText = String(statusText());
  ctx.startChannel = g_startChArm1;
  ctx.totalSpokes = g_spokesTotal;
  ctx.armCount = g_armCount;
  ctx.pixelsPerArm = g_pixelsPerArm;
  ctx.strideMode = static_cast<uint8_t>(g_strideMode);
  ctx.maxArms = MAX_ARMS;
  ctx.maxPixelsPerArm = MAX_PIXELS_PER_ARM;
  ctx.fps = g_fps;
  ctx.brightnessPercent = g_brightnessPercent;

  const String html = buildControlPage(ctx);
  server.send(200, "text/html; charset=utf-8", html);
}
